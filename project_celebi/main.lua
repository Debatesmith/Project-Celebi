...
return function(mod)
    ...
    local function convertMon(game, sourceSave, sourceMon)
        ...
        local goldMon = Mon.new(game.data, sourceMon.species, level, {
            dvs = dvs,
            statExp = statExp,
    ...
    local function mergePokedex(goldSave, sourceSave)
        ...
        for species, has in pairs((sourceSave.pokedex and sourceSave.pokedex.seen) or {}) do
            if has then goldSave.pokedex.seen[species] = true end
        end
        for species, has in pairs((sourceSave.pokedex and sourceSave.pokedex.owned) or {}) do
            if has then
                goldSave.pokedex.seen[species] = true
                goldSave.pokedex.caught[species] = true
            end
        end
    end
    ...
    local function buildLegacySave(game, source, sourceSave)
        ...
        mergePokedex(legacySave, sourceSave)
    ...
    local function ensureCampaignSchema(meta)
        ...
        meta.diagnostics.loadedBuild = "0.1.30"
    ...
    logInfo("Project Celebi v0.1.30 loaded (Release Candidate Gate QoL + Rival/Badge/Balancing Fixes)")
end
